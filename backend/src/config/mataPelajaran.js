...
/**
 * Mengecek apakah mata pelajaran valid untuk kelas tertentu
 * @param {string} mataPelajaran - Nama mata pelajaran
 * @param {string} namaKelas - Nama kelas
 * @returns {boolean}
 */
export const isMataPelajaranValid = (mataPelajaran, namaKelas) => {
  const validMapel = getMataPelajaranByKelas(namaKelas);
  return validMapel.includes(mataPelajaran);
};
